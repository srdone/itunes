...
app.controller('mainCtrl', function($scope, itunesService){
  ...
  $scope.gridOptions = { 
      ...
      columnDefs: [
        ...
        {field: 'Artist', displayName: 'Artist', filter: {term: 'Artist'}},
  ...
  //Also note that that method should be retuning a promise, so you could use .then in this function.
    ...
    $scope.getSongData = function () {
      itunesService.getArtistByName($scope.artist).then(function (data) {
        return data.map(function (current) {
          return {
            Song: current.trackName,
            Artist: current.artistName,
            Collection: current.collectionName,
            Type: current.kind,
            AlbumArt: current.artworkUrl100,
            CollectionPrice: current.collectionPrice
          };
        });
      }).then(function (data) {
        $scope.songData = data;
    ...
    $scope.searchOptions = [
      ...
      {value: 'musicVideo', displayText: 'Music Video'},
      {value: 'movie', displayText: 'Movie'},
      {value: 'musicTrack', displayText: 'Music Track'},
      {value: 'podcast', displayText: 'Podcast'},
      {value: 'tvShow', displayText: 'TV Show'},
    ];
